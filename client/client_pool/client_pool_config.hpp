...
typedef struct participantNode {
  ...
  std::vector<ExternalClient> externalClients;
} ParticipantNode;
...
typedef struct concordClientConfiguration {
  ...
  std::vector<Replica> node;
  std::vector<ParticipantNode> participant_nodes;
} ConcordClientConfiguration;
...
class ClientPoolConfig {
 public:
  ...
  const std::string USE_SIMPLE_CLIENT = "use_simple_client";
  ...
  void ParseConfig(std::istream& config_stream, config::ConcordConfiguration& config);

  void InitializeStructConfig(config::ConcordConfiguration& config);

  void InitializeExternalClient(config::ConcordConfiguration& config);

  void InitializeReplicaConfig(config::ConcordConfiguration& config);

  static std::unique_ptr<bft::communication::ICommunication> ToCommunication(const config::CommConfig& comm_config);

 private:
  // Logger
  logging::Logger logger_;
  ConcordClientConfiguration struct_config_;
};
