...
struct poolMetrics {
  uint64_t requests_counter_ = 0;
  uint64_t executed_requests_counter_ = 0;
  uint64_t rejected_counter_ = 0;
  uint64_t full_batch_counter_ = 0;
  uint64_t partial_batch_counter_ = 0;
  uint64_t first_leg_counter_ = 0;
  uint64_t second_leg_counter_ = 0;
  uint64_t size_of_batch_gauge_ = 0;
  uint64_t clients_gauge_ = 0;
  uint64_t last_request_time_gauge_ = 0;
  uint64_t request_handle_dur_ms_ = 0;
  uint64_t batch_handle_dur_ms_ = 0;
};

class ConcordClientPoolMetrics {
 public:
  ConcordClientPoolMetrics() {}
  virtual ~ConcordClientPoolMetrics() {}

  uint64_t getRequestsCounter() { return metrics_.requests_counter_; }
  uint64_t getExecutedRequestsCounter() { return metrics_.executed_requests_counter_; }
  uint64_t getRejectedCounter() { return metrics_.rejected_counter_; }
  uint64_t getFullBatchCounter() { return metrics_.full_batch_counter_; }
  uint64_t getPartialBatchCounter() { return metrics_.partial_batch_counter_; }
  uint64_t getFirstLegCounter() { return metrics_.first_leg_counter_; }
  uint64_t getSecondLegCounter() { return metrics_.second_leg_counter_; }
  uint64_t getSizeOfBatchGauge() { return metrics_.size_of_batch_gauge_; }
  uint64_t getClientsGauge() { return metrics_.clients_gauge_; }
  uint64_t getLastRequestTimeGauge() { return metrics_.last_request_time_gauge_; }
  uint64_t getRequestHandleDurMS() { return metrics_.request_handle_dur_ms_; }
  uint64_t getBatchHandleDurMS() { return metrics_.batch_handle_dur_ms_; }

  /*virtual void setRequestsCounter(uint64_t) = 0;
  virtual void setExecutedRequestsCounter(uint64_t) = 0;
  virtual void setRejectedCounter(uint64_t) = 0;
  virtual void setFullBatchCounter(uint64_t) = 0;
  virtual void setPartialBatchCounter(uint64_t) = 0;
  virtual void setFirstLegCounter(uint64_t) = 0;
  virtual void setSecondLegCounter(uint64_t) = 0;
  virtual void setSizeOfBatchGauge(uint64_t) = 0;
  virtual void setClientsGauge(uint64_t) = 0;
  virtual void setLastRequestTimeGauge(uint64_t) = 0;
  virtual void setRequestHandleDurMS(uint64_t) = 0;
  virtual void setBatchHandleDurMS(uint64_t) = 0;*/
...
class ConcordClientPool {
 ...
 public:
  ...
  // static std::unique_ptr<ConcordClientPool> create(config_pool::ConcordClientConfiguration config_struct);
  explicit ConcordClientPool(config_pool::ConcordClientConfiguration& config_struct, ConcordClientPoolMetrics* metrics);
  explicit ConcordClientPool(config_pool::ConcordClientConfiguration& config_struct,
 ...
 private:
  ...
  void CreatePool(std::istream& config_stream, config::ConcordConfiguration& config);
  void CreatePool(concord::config_pool::ConcordClientConfiguration& config_stream,
                  config::ConcordConfiguration& config);
  void PrometheusInit(const config::ConcordConfiguration& config, const config_pool::ClientPoolConfig& pool_config);

  void PrometheusInit(const config::ConcordConfiguration& config,
                      config_pool::ConcordClientConfiguration& config_struct);
  ...
  std::string SpanSampler(std::string& span_blob);
  ...
  // Metric
  ConcordClientPoolMetrics* metrics_;
