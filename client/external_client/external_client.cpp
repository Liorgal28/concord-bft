...
void ConcordClient::CreateCommConfig(CommConfig& comm_config,
                                     int num_replicas,
                                     const ConcordClientConfiguration& config_struct) const {
  ...
  if (config_struct.encrypted_config_enabled) {
    comm_config.secretData = config_struct.secret_data;
  } else {
    comm_config.secretData = std::nullopt;
  }
}

void ConcordClient::CreateClient(ConcordClientConfiguration& config_struct, const SimpleClientParams& client_params) {
  ...
  CreateCommConfig(comm_config, num_replicas, config_struct);
  client_id_ = clientId;
  std::set<ReplicaId> all_replicas;
  const std::unordered_map<bft::communication::NodeNum, Replica> node_conf = config_struct.node;
  ...
  if (enable_mock_comm_) {
    ...
    auto comm = ToCommunication(comm_config, config_struct);
    comm_ = std::move(comm);
  }
  ...
  LOG_INFO(logger_, "Client_id=" << client_id_ << " starting communication and creating new bft client instance");
  ClientConfig cfg;
...
std::unique_ptr<bft::communication::ICommunication> ConcordClient::ToCommunication(
    const CommConfig& comm_config, ConcordClientConfiguration& config_struct) {
  if (comm_config.commType == "tls") {
    const auto tls_config = TlsTcpConfig{comm_config.listenIp,
                                         comm_config.listenPort,
                                         static_cast<uint32_t>(comm_config.bufferLength),
                                         comm_config.nodes,
                                         static_cast<int32_t>(comm_config.maxServerId),
                                         comm_config.selfId,
